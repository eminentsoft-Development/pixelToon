...
// Pair testimonials into groups of 2
const testimonialPairs = [];
for (let i = 0; i < testimonials.length; i += 2) {
  testimonialPairs.push(testimonials.slice(i, i + 2));
}

function TestimonialCard({ t, animKey }) {
  return (
    <div
      key={animKey}
      className="animate-[quoteEnter_0.5s_cubic-bezier(0.22,1,0.36,1)_forwards] flex flex-col h-full"
    >
      {/* Top accent line */}
      <div
      ...
      {/* Index */}
      <div
        className="font-['Cormorant_Garamond',serif] text-5xl font-semibold leading-none select-none mb-4"
        style={{
          color: "transparent",
      ...
      {/* Quote */}
      <blockquote className="font-['Cormorant_Garamond',serif] font-light italic text-white/80 leading-relaxed tracking-[0.01em] mb-6 text-[clamp(0.95rem,1.5vw,1.2rem)] flex-1">
        &ldquo;{t.quote}&rdquo;
      </blockquote>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative w-14 h-14 flex-shrink-0">
          ...
          >
            ...
            />
            <defs>
              <linearGradient id="ringGrad2" x1="0" y1="0" x2="56" y2="56">
        ...
        <div className="min-w-0">
          <p className="text-white font-semibold text-sm mb-0.5 font-['Outfit',sans-serif] truncate">
            {t.name}
          </p>
          <p className="text-xs text-orange-400/70 font-['Outfit',sans-serif] truncate">
            {t.course}
          </p>
        </div>

        {/* Stars */}
        <div className="ml-auto hidden sm:flex gap-0.5 flex-shrink-0">
    ...
    >
      ...
      <div className="relative px-4 md:px-16 pt-12 pb-16">
        <div className="max-w-7xl mx-auto">
          {/* Heading row */}
          <div
            className={`flex flex-col md:flex-row md:items-end justify-between gap-4 mb-12 transition-all duration-700 delay-200 ${
              visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
            }`}
          >
            <h2 className="text-white font-['Cormorant_Garamond',serif] font-light leading-[1.05] tracking-[-0.02em] text-[clamp(2rem,4vw,3.5rem)]">
              What our{" "}
            ...
            <div className="flex items-center gap-3 mb-1">
              <span className="text-white/20 text-xs font-mono mr-2">
                {String(activePair + 1).padStart(2, "0")} /{" "}
                {String(testimonialPairs.length).padStart(2, "0")}
              </span>
              <button
                onClick={prev}
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/30 hover:border-orange-400/50 hover:text-orange-400 transition-all duration-300"
                aria-label="Previous"
              >
              ...
              <button
                onClick={next}
                className="w-10 h-10 rounded-full border border-orange-400/40 flex items-center justify-center text-orange-400 hover:bg-orange-400 hover:text-black transition-all duration-300"
                aria-label="Next"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <div
            className={`grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 transition-all duration-700 delay-300 ${
              visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            {pair.map((t, i) => (
              <TestimonialCard key={`${activePair}-${i}`} t={t} animKey={`${activePair}-${i}`} />
            ))}
          </div>
