...
export const ReviewCard = ({ review, setIsPaused, isActive }) => {
  ...
  const handleToggleExpand = (e) => {
    ...
    setIsExpanded(next);
    if (next) setIsPaused(true); // pause carousel when expanding
  };

  return (
    <div className="bg-white p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full min-h-[280px]">
      <div className="flex items-start justify-between mb-6">
        ...
        <Quote className="text-blue-50/50 w-10 h-10 -mt-2" aria-hidden="true" />
      </div>

      <div className="flex-1">
        <p className="text-gray-600 text-[15px] leading-relaxed italic">
          {isExpanded || !shouldShowReadMore ? text : `${text.substring(0, CHAR_LIMIT)}...`}
          {shouldShowReadMore && (
            <button
      ...
      <div className="border-t border-gray-50 flex justify-between items-center pt-4 mt-4">
        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Google Review</span>
        <div className="w-5 h-5" aria-label="Google" role="img">
