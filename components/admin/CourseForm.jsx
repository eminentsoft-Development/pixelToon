...
import {
  Save, Plus, Trash2, Search, GraduationCap, HelpCircle,
  CheckCircle, Star, Wand2, BookOpen, Layers, AlertCircle,
} from "lucide-react";
...
const formSchema = z.object({
  ...
  description: z.string().min(10, "Short summary must be at least 10 characters"),
  images: z
  ...
  canonicalUrl: z.string().url("Must be a valid URL (include https://)").optional().or(z.literal("")),
...
const CourseForm = ({ onSubmit, loading, initialData }) => {
  ...
  const { fields: whyFields, append: appendWhy, remove: removeWhy } =
  ...
  const generateSlug = () => {
    ...
    if (title) {
      ...
      form.setValue("canonicalUrl", `https://pixeltoonzacademy.com/courses/${slug}`, { shouldValidate: true });
    }
  };
      ...
      >
        ...
        {hasErrors && (
          <div className="lg:col-span-12">
            <Alert variant="destructive" className="border-red-200 flex items-center bg-red-50">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription className="text-sm mt-1.5 text-red-700">
                Please fix the highlighted errors before saving. All required fields must be completed.
              </AlertDescription>
            </Alert>
          </div>
        )}
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden">
            <SectionHeader icon={BookOpen} iconColor="text-slate-600" title="Course Overview" />
                    ...
                    <FormItem className="flex-1">
                      ...
                      <FormControl>
                        <Input
                          ...
                          className={inputCn(fieldState.error, "h-10 text-sm font-mono")}
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        ...
                        rows={3}
                        className={inputCn(fieldState.error, "text-sm resize-none")}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
          ...
          <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden">
            <SectionHeader
              icon={CheckCircle} iconColor="text-green-600" title="Why This Course?"
              action={<AddButton onClick={() => appendWhy({ value: "" })} label="Add Reason" />}
            />
            <CardContent className="space-y-3 pt-0">
              {whyFields.length === 0 && <p className="text-xs text-slate-400 text-center py-3">No reasons added yet.</p>}
          ...
          <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden">
            <SectionHeader
              icon={GraduationCap} iconColor="text-indigo-600" title="What You Will Acquire"
              action={<AddButton onClick={() => appendAcquire({ value: "" })} label="Add Skill" />}
            />
            <CardContent className="space-y-3 pt-0">
              {acquireFields.length === 0 && <p className="text-xs text-slate-400 text-center py-3">No skills added yet.</p>}
          ...
          {/* ── SEO Settings ── */}
          <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden">
            <SectionHeader icon={Search} iconColor="text-sky-600" title="SEO Settings" />
            <CardContent className="space-y-5 pt-0">
              ...
              <FormField
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    <FormLabel className={labelCn(fieldState.error)}>Meta Title<Req /></FormLabel>
                    <FormControl>
                      <Input
              ...
              <FormField
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    <FormLabel className={labelCn(fieldState.error)}>Canonical URL</FormLabel>
                    <FormControl>
                      <Input className={inputCn(fieldState.error, "h-10 text-sm font-mono")} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field, fieldState }) => (
                  <FormItem>
                    <FormLabel className={labelCn(fieldState.error)}>Meta Description<Req /></FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-sm font-medium text-slate-700">Meta Keywords</FormLabel>
                    <FormControl>
                      <Input
          ...
          <div className="sticky top-6 space-y-5">
            <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden">
              <CardContent className="p-5 space-y-5">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Control visibility and promotion</p>
                </div>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex items-center space-x-3 space-y-0 rounded-lg border border-slate-100 bg-slate-50 p-3">
                      ...
                      <div className="space-y-0.5 leading-none">
                        <FormLabel className="text-sm font-medium text-slate-700 cursor-pointer">Published</FormLabel>
                        <FormDescription className="text-[11px] text-slate-400">Visible on the website</FormDescription>
                      </div>
                    </FormItem>
                  )}
                />
                    ...
                    <FormItem className="flex items-center space-x-3 space-y-0 rounded-lg border border-slate-100 bg-slate-50 p-3">
                      ...
                      <div className="space-y-0.5 leading-none">
                        <FormLabel className="text-sm font-medium text-slate-700 cursor-pointer flex items-center gap-1.5">
                          Featured <Star size={11} className="fill-amber-400 text-amber-400" />
                        </FormLabel>
                        <FormDescription className="text-[11px] text-slate-400">Highlighted in home sliders</FormDescription>
                      </div>
                    </FormItem>
                  )}
                />
                ...
                >
                  <Save size={15} />
                  {loading ? "Saving…" : initialData ? "Update Course" : "Save Course"}
                </Button>

                {hasErrors && (
                  <p className="text-xs text-red-500 text-center">Fix all errors above before saving.</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border border-slate-200 shadow-none rounded-xl overflow-hidden bg-slate-50">
              <CardContent className="p-5">
                <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Tips</h4>
                <ul className="space-y-2 text-xs text-slate-500">
                  {[
                    <><strong className="text-slate-600">Generate</strong> auto-creates a slug from the title.</>,
                    ...
                    <li key={i} className="flex gap-2"><span className="text-slate-300 shrink-0">•</span>{tip}</li>
                  ))}
                </ul>
              </CardContent>
            </Card>
